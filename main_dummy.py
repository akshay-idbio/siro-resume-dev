...
app = FastAPI(
    title=f"{settings.app_title} - DUMMY LOAD TEST",
...
async def startup_event():
    print("=" * 90, flush=True)
    print("[DUMMY-LOAD-TEST] WARNING: DUMMY BACKEND ACTIVE - CLAUDE WILL NOT BE CALLED", flush=True)
    print(
        f"[DUMMY-LOAD-TEST] delay={DUMMY_PROCESSING_DELAY_SECONDS}s per resume; "
        f"parallel_limit={os.getenv('MAIN_AI_PARALLEL_RESUMES', '5')}",
        flush=True,
    )
    print("=" * 90, flush=True)
...
# =========================================================
# DUMMY LOAD-TEST PROCESSOR
# =========================================================

DUMMY_PROCESSING_DELAY_SECONDS = float(
    os.getenv("DUMMY_PROCESSING_DELAY_SECONDS", "0.25")
)


async def process_one_resume_for_db_job_dummy(
    job_id: str,
    user_id: str,
    resume_path: str,
    resume_filename: str,
    ...
    """
    Load-test processor used by main_dummy.py.

    It intentionally does not open the resume file and never calls Claude.
    It still exercises MongoDB status updates, progress counters, result storage,
    concurrency, final Excel generation, polling, and download flow.
    """
    ...
    async with semaphore:
        ...
        print(
            f"[DUMMY-LOAD-TEST] START "
            f"job_id={job_id} resume={index}/{total_files} "
            f"filename={filename}",
            flush=True,
        )
        ...
        await update_job_progress(
            job_id=job_id,
            current_file=filename,
            message=f"Dummy processing resume {index} of {total_files}",
        )

        try:
            delay = max(0.0, DUMMY_PROCESSING_DELAY_SECONDS)

            print(
                f"[DUMMY-LOAD-TEST] SIMULATE "
                f"job_id={job_id} resume={index}/{total_files} "
                f"delay={delay}s",
                flush=True,
            )
            ...
            if requirements_df is None or requirements_df.empty:
                raise ValueError("Requirement Excel contains no valid rows")

            requirement_index = (index - 1) % len(requirements_df)
            req = requirements_df.iloc[requirement_index].fillna("").to_dict()
            request_id = clean_cell(req.get("Request-ID"))

            candidate_info = {
                "candidate_name": f"Dummy Candidate {index}",
                "candidate_phone": f"900000{index:04d}",
                "candidate_email": f"dummy.candidate.{index}@loadtest.local",
                ...
                "candidate_location_evidence": "Mumbai, India",
                "candidate_location_confidence": "high",
                "candidate_total_experience_years": 5,
                "candidate_current_ctc": None,
                "candidate_expected_ctc": None,
                "candidate_notice_period": None,
                "candidate_skills": ["Python", "SQL", "Load Testing"],
                "recent_job_title": "Dummy Engineer",
                "profile_summary": "Synthetic candidate generated for load testing only.",
            }
            ...
            match_result = {
                ...
                "missing_skills": [],
                "matched_core_requirements": ["Synthetic load-test match"],
                "missing_core_requirements": [],
                "location_match_evidence": "Dummy Mumbai candidate vs requirement location",
                "reason": (
                    "Dummy load-test result. No resume parsing and no Claude API call was made."
                ),
            }
            ...
            result_output_rows = [output_row]
            result_tracker_rows = [tracker_row]
            resume_seconds = round(time.time() - resume_start_time, 2)
            ...
            await update_job_progress(
                ...
                successful_delta=1,
                current_file=filename,
                message=f"Dummy processed resume {index} of {total_files}",
            )

            print(
                f"[DUMMY-LOAD-TEST] SUCCESS "
                f"job_id={job_id} resume={index}/{total_files} "
                f"filename={filename} duration={resume_seconds}s "
            ...
            await update_job_progress(
                ...
                failed_delta=1,
                current_file=filename,
                message=f"Dummy failed resume {index} of {total_files}",
            )
...
async def process_db_backed_main_job_background(
    ...
    try:
        ...
        print(
            f"[DUMMY-LOAD-TEST] JOB START "
            f"job_id={job_id} resumes={total_files} "
            f"requirement_rows={len(requirements_df)} "
            f"parallel_limit={parallel_limit} "
            f"delay={DUMMY_PROCESSING_DELAY_SECONDS}s",
            flush=True,
        )

        tasks = [
            process_one_resume_for_db_job_dummy(
                job_id=job_id,
                user_id=user_id,
        ...
        completed_count = sum(
            1 for item in task_results if item.get("status") == "completed"
        )
        failed_count = sum(
            1 for item in task_results if item.get("status") == "failed"
        )
        skipped_count = sum(
            1 for item in task_results if item.get("status") == "skipped"
        )

        print(
            f"[DUMMY-LOAD-TEST] ALL TASKS FINISHED "
            f"job_id={job_id} completed={completed_count} "
